...
const match = (str: string, include: string[], exclude: string[]): boolean => {
  return (include.length === 0 || isMatch(str, include)) &&
        (exclude.length === 0 || !isMatch(str, exclude))
}
...
export const getTables = async (profile: string, include: string[], exclude: string[]): Promise<TableItem[]> => {
  ...
  const tables = await Promise.all(TableNames.map(async (tablename: string) => {
    const { stdout } = await exec('aws', ['--profile', `${profile}`,  'dynamodb', 'describe-table', '--table-name', tablename])
